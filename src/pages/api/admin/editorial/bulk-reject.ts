...
export const POST: APIRoute = async ({ request, cookies }) => {
  try {
    ...
    // Reject providers (set editor_approved_for_premium to false)
      ...
      if (error) {
        ...
        if (user.id) {
          const actions = providerIds.map(id => ({
            ...
            entity_id: id,
            description: 'Rejected for premium (bulk editorial review)'
          }));
    ...
    return new Response(JSON.stringify({
      ...
      failed: failedCount,
      errors,
      message: `Rechazados: ${successCount} | Errores: ${failedCount}`
    }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' }
    });
  } catch (error) {
    console.error('Error in POST /api/admin/editorial/bulk-reject:', error);
